...
def start_algorithm(method = 'qaoa', state = 0, p_lst = list(range(1, 7 + 1)), qaoa_st = None, mix_matrix = qt.sigmax(), \
    ...
    if(method == 'diag'):
        ...
        for p in p_lst:
            ch.find_optimal_distance_qaoa(p, qaoa_st, sgd_method, grad_desc_opts)
    elif(method == 'average'):
...
try:
    ...
    state = args.state
    qaoa_st = True if state > 0 else None
    ...
    qaoa_param_gs_basin = ['qaoa', state, p_lst, qaoa_st, mixing_dict[args.mixing], 'basin', basin_opts] + atoms
    qaoa_param_gs_adam = ['qaoa', state, p_lst, qaoa_st, mixing_dict[args.mixing], 'adam', adam_opts] + atoms
    qaoa_param_gs_average = ['average', state, p_lst, qaoa_st, mixing_dict[args.mixing], None, None] + atoms
